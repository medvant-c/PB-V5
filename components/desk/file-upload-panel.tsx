...
function FileUploadPanel({ tab, relatedId = null, className }: FileUploadPanelProps) {
  const [files, setFiles] = useState<DeskFileRecord[]>([]);
  ...
  const loadFiles = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/desk-files?${params.toString()}`);
  ...
  async function uploadFile(file: File) {
    ...
    try {
      ...
      const res = await fetch("/api/desk-files", { method: "POST", body: formData });
      const data = await res.json();
      if (res.ok) {
  ...
  async function deleteFile(id: string) {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/desk-files/${id}`, { method: "DELETE" });
      if (res.ok) {
  ...
  return (
    <div className={cn("space-y-3", className)}>
      ...
      >
        ...
        <p className="text-[11px] text-text-secondary">PDF, DOC(X), XLS(X), PNG, JPG — до 20MB</p>
      </div>
