...
export async function POST(req: NextRequest) {
  ...
  const client = await prisma.client.findUnique({ where: { email: email.trim().toLowerCase() } });
  if (client) {
    const token = await createAccountToken(client.id, "reset");
    const origin = req.nextUrl.origin;
    await sendPasswordResetEmail(client.email, `${origin}/account/activate?token=${token}`);
  }
