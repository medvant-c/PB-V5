...
export async function PATCH(req: NextRequest, { params }: RouteParams) {
  ...
  // Старая схема ("курсовая разница целиком Владу, мимо менеджера") больше
  // не используется в интерфейсе — заменена реальными деньгами через Кассу
  // (expense-order + create-payment), где курсовая разница растворяется в
  // обычной прибыли сделки наравне с остальным (см. PB-V5 chat 2026-08-26).
  // Этот роут остаётся рабочим только для ПРОДОЛЖЕНИЯ уже начатых по старой
  // схеме сделок (buyoutFactConfirmed уже true) — например, поправить факт
  // задним числом — но не может ЗАПУСТИТЬ старую схему на новом просчёте.
  if (!quote.buyoutFactConfirmed) {
    return Response.json(
      {
        error:
          "Эта сделка ещё не на старой схеме подтверждения — используйте «Расходный ордер» на карточке просчёта и «Приходный ордер» для оплаты клиента.",
      },
      { status: 400 },
    );
  }
